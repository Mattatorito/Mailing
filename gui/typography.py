...
Использование:
    from .typography import apply_heading_style, apply_text_style
    label = QLabel("Dashboard")
    apply_heading_style(label, level="h1")
...
def font_px(key: str) -> int:
    """Возвращает размер шрифта (pt) для ключа типографической шкалы.

    Qt традиционно оперирует pointSize для QFont. Мы оставляем значения как есть.
    """
...
def apply_heading_style(widget: QWidget, level: str = 'h1', weight: int = QFont.DemiBold):
    ...
    weight по умолчанию DemiBold (между Medium и Bold) – визуально легче.
    """
    key = HEADING_MAP.get(level, 'title1')
    size = font_px(key)
...
def apply_text_style(widget: QWidget, kind: str = BODY_DEFAULT, weight: int | None = None):
...
__all__ = [
    'apply_heading_style', 'apply_text_style', 'font_px', 'make_page_title'
]

def make_page_title(text: str, level: str = 'h1') -> QLabel:
    """Фабрика стандартизированного заголовка раздела.
    ...
    # Универсальные отступы: сверху 0, снизу 16 (регулируемо), слева/справа 0
    lbl.setStyleSheet("margin: 0 0 20px 0; padding: 0;")
    return lbl
