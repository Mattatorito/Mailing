...
import time
        ...
        from . import styles
    from .typography import make_page_title
...
class ErrorDotDelegate(QStyledItemDelegate):
    ...
    # Определяем цвет по количеству
        if num == 0:color = QColor("#10B981")  # зеленый - нет ошибок
        elif num < 20:color = QColor("#F59E0B")  # оранжевый - немного
        elif num < 60:color = QColor("#F97316")  # ярко-оранжевый
    else:color = QColor("#EF4444")  # красный - много
    ...
    # Текст числа правееpainter.setPen(QColor("#FFFFFF"))
    painter.drawText(
        dot_x + dot_size + 6,
        r.y(),
        r.width() - dot_size - 12,
...
# Расширенные секции
ENHANCED_SECTION_KEYS = ["section_dashboard","""section_campaigns","""section_webhook_manager","""section_system_monitor","""section_config_manager","""section_template_editor","""section_statistics","""section_recipients","""section_logs","""section_settings",""]
...
class WebhookServerManager(QThread):
    ...
    def start_server(self, port = 8000):"""Запуск webhook сервера"""
    """Выполняет start server."""
        if self.running:
            return

        try:
        cmd = [sys.executable,"
        ""-m","""uvicorn","""mailing.webhook_server:app","""--host","""0.0.0.0","""--port",str(port),"
        ""--reload",
        ]
        ...
        except Exception as e:self.server_error.emit(f"Ошибка запуска сервера: {e}")

    def stop_server(self):"""Остановка webhook сервера"""
    """Выполняет stop server."""
    ...
    def run(self):"""Мониторинг вывода сервера"""
    """Выполняет run."""
        if not self.process:
            return
...
class SystemMonitorWidget(QWidget):"""Виджет для мониторинга системы"""
    """Класс SystemMonitorWidget."""
    ...
    # Заголовок страницы (унифицированный)title = make_page_title("📊 System Monitor")
    layout.addWidget(title)

    # Карточки основной статистики
    cards_layout = QHBoxLayout()

    self.db_card = StatsCard("База данных","""Загрузка...",0.0,"
        "accent = QColor("#8B5CF6"),
        parent = None,theme_manager = self.theme_manager,"
        "glyph="🗄","")  # Фиолетовый
    self.quota_card = StatsCard(
        "Квоты","""Загрузка...",0.0,"
        "accent = QColor("#F59E0B"),
        parent = None,theme_manager = self.theme_manager,"
        "glyph="📊","")  # Янтарный
    self.webhook_card = StatsCard(
        "Webhook сервер","""Загрузка...",0.0,"
    ...
    self.deliveries_table.setColumnCount(3)self.deliveries_table.setHorizontalHeaderLabels(["Email",
        "Успех", "Ошибка"])
    ...
    self.deliveries_table.setSizePolicy(
        QSizePolicy.Expanding, QSizePolicy.Expanding
    )
    deliveries_layout.addWidget(self.deliveries_table)

    # Таблица webhook событийevents_group = QGroupBox("Webhook события")
    ...
    self.events_table.setColumnCount(3)self.events_table.setHorizontalHeaderLabels(["Email",
        "Подпись", "Время"])
    header = self.events_table.horizontalHeader()
    header.setSectionResizeMode(
        0, QHeaderView.Stretch
    )  # Email растягивается полностью
    ...
    # Системная информацияsystem_info_group = QGroupBox("Информация о системе")
    ...
    self.system_info_text.setSizePolicy(
        QSizePolicy.Expanding, QSizePolicy.Expanding
    )
    ...
    def update_stats(self):"""Обновление статистики"""
    """Выполняет update stats."""
        try:
        # Статистика БД
        stats = self.delivery_repo.stats()total = stats["total"]success_rate = (stats["success"] / max(total, 1)) * 100self.db_card.setValue(f"{total} доставок")
        self.db_card.setProgress(success_rate / 100)

        # Квоты
        used = self.quota.used()
        limit = self.quota.limit
        quota_percent = (used / limit) * 100self.quota_card.setValue(f"{used}/{limit}")
        self.quota_card.setProgress(quota_percent / 100)

        # Проверка webhook сервера
            try:

                with httpx.Client(timeout = 1.0) as client:response = client.get("http://localhost:8000/health")
                    if response.status_code == 200:self.webhook_card.setValue("Работает")
                    self.webhook_card.setProgress(1.0)
                else:self.webhook_card.setValue("Ошибка")
                    self.webhook_card.setProgress(0.0)
            except:self.webhook_card.setValue("Остановлен")
            self.webhook_card.setProgress(0.0)
        ...
        except Exception as e:logging.error(f"Ошибка обновления статистики: {e}")

    def update_deliveries_table(self):"""Обновление таблицы доставок"""
    """Выполняет update deliveries table."""
        try:
        ...
        self.deliveries_table.setRowCount(len(deliveries))
            ...
            self.deliveries_table.setItem(row,0, QTableWidgetItem(delivery["email"])
                ...
                # Statusstatus = "✅ Успешно" if delivery["success"] else "❌ Ошибка"
                status_item = QTableWidgetItem(status)if delivery["success"]:status_item.setBackground(QColor("#10B981"))
            else:status_item.setBackground(QColor("#EF4444"))
            self.deliveries_table.setItem(row, 1, status_item)

            # Errorerror_text = delivery["error"] or "-"
                if len(error_text) > 80:error_text = error_text[:77] + "..."
            self.deliveries_table.setItem(row, 2, QTableWidgetItem(error_text))

        except Exception as e:logging.error(f"Ошибка обновления таблицы доставок: {e}")

    def update_events_table(self):"""Обновление таблицы событий"""
    """Выполняет update events table."""
        try:
        ...
        self.events_table.setRowCount(len(events))
            ...
            self.events_table.setItem(
                row,0,"
        "QTableWidgetItem(str(event.get("recipient") or event.get("event_type") or "")
                ),
            )
            self.events_table.setItem(
                row,1,"
            "QTableWidgetItem("✅" if event.get("signature_valid") else "❌"),"")
            self.events_table.setItem(
                row, 2, QTableWidgetItem(str(event.get("created_at", "")))
            )

        except Exception as e:logging.error(f"Ошибка обновления таблицы событий: {e}")

    def update_system_info(self):"""Обновление системной информации"""
        try:info_text = f"""
Конфигурация:
...
База данных:
• Путь: {settings.sqlite_db_path}• Всего доставок: {self.delivery_repo.stats()['total']}
...
"""
        ...
        except Exception as e:logging.error(f"Ошибка обновления системной информации: {e}")

class ConfigManagerWidget(QWidget):"""Виджет для управления конфигурацией"""
    """Класс ConfigManagerWidget."""

    def __init__(self, theme_manager: ThemeManager):
    """Инициализирует объект."""
    super().__init__()
    self.theme_manager = theme_managerself.env_file_path = Path(".env")
    self.init_ui()
    self.load_config()
    ...
    # Заголовок страницыtitle = make_page_title("⚙️ Config Manager")
    layout.addWidget(title)

    # Предупреждениеwarning = QLabel("⚠️ Изменения требуют перезапуска приложения")warning.setStyleSheet("color: orange; margin-bottom: 20px;")
    ...
    config_sections = [
        ...
        "("Общие настройки",""[
                ("DAILY_EMAIL_LIMIT","Дневной лимит писем","number"),
                    ...
                    ""("MAX_RETRIES","Максимум повторов","number"),""("CONCURRENCY",
                    "Параллельность","number"),""("SQLITE_DB_PATH","Путь к БД",
                    "text"),""("LOG_LEVEL","Уровень логирования","combo"),
            ],),"
        "]
        ...
        scroll_layout.addRow(section_label)
            ...
            for env_key, label, field_type in fields:if field_type == "password":
                field = QLineEdit()
                    field.setEchoMode(QLineEdit.Password)elif field_type == "number":
                field = QSpinBox()
                    field.setRange(0, 100000)elif field_type == "combo" and env_key == "LOG_LEVEL":
                field = QComboBox()field.addItems(["DEBUG","INFO","WARNING","ERROR", "CRITICAL"])
            else:
                field = QLineEdit()

            self.config_fields[env_key] = fieldscroll_layout.addRow(label + ":", field)
    ...
    def load_config(self):"""Загрузка текущей конфигурации"""
    """Выполняет load config."""
        try:
        # Загружаем из .env файла
        env_vars = {}
            if self.env_file_path.exists():with open(self.env_file_path, "r") as f:
                    for line in f:
                        line = line.strip()if line and not line.startswith("#") and "=" in line:key,
                        value = line.split("=", 1)env_vars[key.strip()] = value.strip().strip("\"'")

        # Заполняем поля
            for env_key,field in self.config_fields.items():value = env_vars.get(env_key, "")
...
QMessageBox.information(self, "Успех", "Конфигурация загружена")
        ...
        except Exception as e:QMessageBox.critical(self,"Ошибка", f"Ошибка загрузки конфигурации: {e}")

    def save_config(self):"""Сохранение конфигурации в .env файл"""
    """Выполняет save config."""
        try:
        lines = []lines.append("# Конфигурация системы массовой рассылки")lines.append("# Создано GUI приложением")lines.append("")
current_section = ""
            for env_key, field in self.config_fields.items():
                # Определяем секциюif env_key.startswith("RESEND_") and current_section != "RESEND":lines.append("# Resend настройки")current_section = "RESEND"
                elif (not env_key.startswith(("RESEND_",))and current_section != "GENERAL"
            ):lines.append("# Общие настройки")current_section = "GENERAL"
            ...
            # Добавляем в файл только если есть значение
                if value:lines.append(f"{env_key}={value}")

            # Сохраняем файлwith open(self.env_file_path, "w") as f:f.write("\\n".join(lines))

        QMessageBox.information(self,"
        ""Успех","""Конфигурация сохранена!\\n\\nПерезапустите приложение для применения изменений.","")

        except Exception as e:
        QMessageBox.critical(self, "Ошибка", f"Ошибка сохранения конфигурации: {e}")

    def reset_config(self):"""Сброс конфигурации к значениям по умолчанию"""
    """Выполняет reset config."""
    reply = QMessageBox.question(self,"
        ""Подтверждение","""Вы уверены, что хотите сбросить все настройки?",QMessageBox.Yes | QMessageBox.No,"
        ")
...
class WebhookManagerWidget(QWidget):"""Виджет для управления webhook сервером"""
    """Класс WebhookManagerWidget."""
    ...
    # Заголовок страницыtitle = make_page_title("🔗 Webhook Manager")
    layout.addWidget(title)

    # Управление серверомserver_group = QGroupBox("Управление Webhook сервером")
    server_layout = QVBoxLayout(server_group)

    # Статус и кнопки
    status_layout = QHBoxLayout()
self.status_label = QLabel("Статус: Остановлен")self.status_label.setStyleSheet("font-weight: bold;")
    ...
    # Настройки порта
    port_layout = QHBoxLayout()port_layout.addWidget(QLabel("Порт:"))
    ...
    self.server_log.setMaximumHeight(200)server_layout.addWidget(QLabel("Лог сервера:"))
    server_layout.addWidget(self.server_log)

    layout.addWidget(server_group)

    # Тестирование endpointstest_group = QGroupBox("Тестирование Webhook Endpoints")
    ...
    self.test_results.setMaximumHeight(150)test_layout.addWidget(QLabel("Результаты тестов:"))
    test_layout.addWidget(self.test_results)

    layout.addWidget(test_group)

    def start_server(self):"""Запуск сервера"""
    """Выполняет start server."""
    port = self.port_spinbox.value()
    self.server_manager.start_server(port)self.server_log.append(f"Запуск сервера на порту {port}...")

    def stop_server(self):"""Остановка сервера"""
    """Выполняет stop server."""
    self.server_manager.stop_server()self.server_log.append("Остановка сервера...")

    def test_server(self):"""Тест доступности сервера"""
    """Тест для server."""
    threading.Thread(target = self._test_server_async, daemon = True).start()

    def _test_server_async(self):"""Асинхронный тест сервера"""
    """Выполняет  test server async."""
        try:
        port = self.port_spinbox.value()
            try:

                with httpx.Client(timeout = 5.0) as client:response = client.get(f"http://localhost:{port}/health")
                    if response.status_code == 200:
                    QTimer.singleShot(0, lambda: self.test_results.append("✅ Сервер доступен")
                    )
                else:
                    QTimer.singleShot(0,"
        "lambda: self.test_results.append(f"❌ Сервер вернул статус: {response.status_code}"),"
        ")
            except ImportError:
            # Используем стандартную библиотеку если httpx недоступен
                ...
                try:
                    with urllib.request.urlopen(f"http://localhost:{port}/health", timeout = 5
                        ...
                        QTimer.singleShot(0,"
        "lambda: self.test_results.append("✅ Сервер доступен"),
                        )
                    else:
                        QTimer.singleShot(0,"
        "lambda: self.test_results.append(f"❌ Сервер вернул статус: {response.status}"
                            ),
                        )
                except urllib.error.URLError as e:
                QTimer.singleShot(0,"
        "lambda: self.test_results.append(f"❌ Ошибка подключения: {e}"),"")
        except Exception as e:
        QTimer.singleShot(
            0, lambda: self.test_results.append(f"❌ Неожиданная ошибка: {e}")
        )

        traceback.print_exc()

    def test_health(self):"""Тест health endpoint"""
    """Тест для health."""
    threading.Thread(target = self._test_health_async, daemon = True).start()

    def _test_health_async(self):"""Асинхронный тест health"""
    """Выполняет  test health async."""
        try:
        port = self.port_spinbox.value()
            try:

                with httpx.Client(timeout = 5.0) as client:response = client.get(f"http://localhost:{port}/health")message = f"Health: {response.status_code} - {response.text}"
                QTimer.singleShot(
                    0, lambda msg = message: self.test_results.append(msg)
                )
            except ImportError:

                try:
                    with urllib.request.urlopen(f"http://localhost:{port}/health", timeout = 5
                ) as response:data = response.read().decode("utf-8")self.test_results.append(f"Health: {response.status} - {data}")
                except urllib.error.URLError as e:self.test_results.append(f"Health test error: {e}")
        except Exception as e:self.test_results.append(f"Health test error: {e}")

        traceback.print_exc()

    def test_resend_webhook(self):"""Тест Resend webhook"""
    """Тест для resend webhook."""
    threading.Thread(target = self._test_resend_async, daemon = True).start()

    def _test_resend_async(self):"""Асинхронный тест Resend webhook"""
    """Выполняет  test resend async."""
        try:
        port = self.port_spinbox.value()
        test_data = {"type": "email.delivered","""data": {"email_id": "gui_test_123", "to": "test@example.com"},""}

            try:

                with httpx.Client(timeout = 5.0) as client:
                response = client.post(
                    f"http://localhost:{port}/resend/webhook",json = test_data,"
        "headers={"Content-Type": "application/json"},"")
                self.test_results.append(
                    f"Resend webhook: {response.status_code} - {response.text}"
                )
            except ImportError:

                try:data = json.dumps(test_data).encode("utf-8")
                req = urllib.request.Request(f"http://localhost:{port}/resend/webhook",
                    data = data,"
        "headers={"Content-Type": "application/json"},"")
                ...
                except urllib.error.URLError as e:self.test_results.append(f"Resend webhook error: {e}")
        except Exception as e:self.test_results.append(f"Resend webhook error: {e}")

        traceback.print_exc()

    def view_events(self):"""Просмотр событий"""
    """Выполняет view events."""
    threading.Thread(target = self._view_events_async, daemon = True).start()

    def _view_events_async(self):"""Асинхронный просмотр событий"""
    """Выполняет  view events async."""
        try:
        port = self.port_spinbox.value()
            try:

                with httpx.Client(timeout = 5.0) as client:response = client.get(f"http://localhost:{port}/events?limit = 5")
                    if response.status_code == 200:
                    events = response.json()self.test_results.append(f"События ({len(events)}):")
                ...
                try:
                    with urllib.request.urlopen(f"http://localhost:{port}/events?limit = 5", timeout = 5
                ) as response:data = response.read().decode("utf-8")
                    events = json.loads(data)self.test_results.append(f"События ({len(events)}):")
                        for event in events[:3]:
                        self.test_results.append(f"  • {event.get('event_type')} - {event.get('recipient')}"
                        )
                except urllib.error.URLError as e:self.test_results.append(f"Events error: {e}")
                except json.JSONDecodeError as e:self.test_results.append(f"JSON decode error: {e}")
        except Exception as e:self.test_results.append(f"Events error: {e}")

        traceback.print_exc()

    def on_server_started(self):"""Обработчик запуска сервера"""self.status_label.setText("Статус: Запущен")self.status_label.setStyleSheet("font-weight: bold; color: green;")
    """Выполняет on server started."""
    self.start_button.setEnabled(False)
    self.stop_button.setEnabled(True)

    def on_server_stopped(self):"""Обработчик остановки сервера"""self.status_label.setText("Статус: Остановлен")self.status_label.setStyleSheet("font-weight: bold; color: red;")
    """Выполняет on server stopped."""
    self.start_button.setEnabled(True)
    self.stop_button.setEnabled(False)

    def on_server_error(self, error_msg):"""Обработчик ошибки сервера"""self.server_log.append(f"❌ Ошибка: {error_msg}")

    def on_server_output(self, output):"""Обработчик вывода сервера"""
    """Выполняет on server output."""
    self.server_log.append(output)

class SquareCheckBox(QCheckBox):"""Кастомный чекбокс: пустой квадрат + белая галочка без заливки."""
...
def __init__(self, text: str = "", parent = None, box_size: int = 18):
    ...
    # Убираем стандартные отступы чтобы контролировать самиself.setStyleSheet("QCheckBox{spacing:8px;}")
    ...
    # Цветаborder_color = QColor("#4A5568")
        if self.isChecked():border_color = QColor("#6366F1")
        if self._hover:
        # Лёгкое подсвечивание границы при hover
            if not self.isChecked():border_color = QColor("#6366F1")
...
class EnhancedMainWindow(QMainWindow):"""Расширенное главное окно с полным управлением системой"""
    """Класс EnhancedMainWindow."""
    ...
    def _init_ui(self):"""Инициализация интерфейса"""
    """Выполняет  init ui."""
    root = QWidget()
    ...
    # Логотип/заголовокtitle_label = QLabel("📧 Mail System")
    ...
    section_items = [("📊 Dashboard","section_dashboard"),""("🚀 Рассылки",
        "section_campaigns"),""("🔗 Webhook","section_webhook_manager"),
        ""("📈 Мониторинг","section_system_monitor"),""("⚙️ Конфигурация",
        "section_config_manager"),""("📝 Шаблоны","section_template_editor"),
        ""("📋 Статистика","section_statistics"),""("👥 Получатели",
        "section_recipients"),""("📄 Логи","section_logs"),""("🔧 Настройки",
        "section_settings"),""]
    ...
    def _create_dashboard_page(self):"""Создание страницы Dashboard"""
    """Выполняет  create dashboard page."""
    page = QWidget()
    layout = QVBoxLayout(page)apply_margins(layout,"page")apply_spacing(layout, "lg")

    # Заголовокtitle = QLabel("📊 Dashboard")apply_heading_style(title, "h1")title.setStyleSheet("color: #6366F1; padding-bottom: 8px; margin-bottom: 16px;")
    ...
    self.dashboard_sent_card = StatsCard("Отправлено","""0",0.0,"
        "accent = QColor("#6366F1"),
        parent = None,theme_manager = self.theme_manager,"
        ")
    self.dashboard_success_card = StatsCard("Успешно","""0%",0.0,"
        "accent = QColor("#10B981"),
        parent = None,theme_manager = self.theme_manager,"
        ")
    self.dashboard_failed_card = StatsCard("Ошибок","""0",0.0,"
        "accent = QColor("#EF4444"),
        parent = None,theme_manager = self.theme_manager,"
        ")
    self.dashboard_quota_card = StatsCard("Квота","""0/100",0.0,"
        "accent = QColor("#06B6D4"),
        parent = None,theme_manager = self.theme_manager,"
        ")
    ...
    layout.addSpacing(
        24
    )  # Увеличиваем отступ между карточками и быстрыми действиями
...
actions_group = QGroupBox("Быстрые действия")
    ...
    layout.addSpacing(
        24
    )  # Увеличиваем отступ между быстрыми действиями и активностью
    ...
    activity_container_layout.setContentsMargins(
        8, 8, 8, 12
    )  # Уменьшаем внешние отступы

    # Последняя активностьactivity_group = QGroupBox("Последняя активность")
    # Делаем заголовок видимым с небольшим отступом от границы
    ...
    def _create_campaigns_page(self):"""Создание страницы рассылок (упрощённая версия)"""
    """Выполняет  create campaigns page."""
    page = QWidget()
    main_layout = QVBoxLayout(page)apply_margins(main_layout,
        "section")apply_spacing(main_layout, "md")

title = make_page_title("🚀 Управление рассылками", level="h1")title.setStyleSheet("margin: 0 0 24px 0; color: #6366F1;")
    title.setAlignment(Qt.AlignCenter)
    ...
    self.recipients_path = FloatingTextField("Файл получателей",""","""recipients.csv",
        parent = None,theme_manager = self.theme_manager,"
        ")
    ...
    self.template_path = FloatingTextField("Файл шаблона",""","""template.html.j2",
        parent = None,theme_manager = self.theme_manager,"
        ")
    ...
    parallel_group.setMinimumWidth(0)
    parallel_group.setMaximumHeight(80)  # Уменьшили высоту
    parallel_layout = QHBoxLayout(parallel_group)  # Горизонтальный layout
    parallel_layout.setContentsMargins(8, 16, 8, 16)
    parallel_layout.setSpacing(6)
    parallel_layout.setAlignment(Qt.AlignVCenter)
parallel_label = QLabel("Параллельность")
    parallel_label.setStyleSheet("font-weight: 600; color: #E5E7EB; font-size: 13px; padding-right: 4px;"
    )
    parallel_label.setAlignment(Qt.AlignVCenter)
    parallel_label.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Preferred)
    parallel_layout.addWidget(parallel_label)

    self.concurrent_input = QLineEdit()self.concurrent_input.setText("10")
    self.concurrent_input.setAlignment(Qt.AlignCenter)
    self.concurrent_input.setValidator(QIntValidator(1, 100, self))
    self.concurrent_input.setMinimumHeight(28)
    self.concurrent_input.setFixedWidth(64)
    self.concurrent_input.setStyleSheet("""
        QLineEdit {
    ...
    parallel_layout.addWidget(self.concurrent_input)
    ...
    settings_layout.addWidget(parallel_group, 0, Qt.AlignLeft)
...
test_label = QLabel("Тестовый режим:")test_label.setStyleSheet("font-weight: bold; color: white; font-size: 14px;")
...
self.progress_label = QLabel("Готов к запуску")
    ...
    def _create_template_editor_page(self):"""Создание страницы редактора шаблонов"""
    """Выполняет  create template editor page."""
    page = QWidget()
    layout = QVBoxLayout(page)apply_margins(layout,"page")apply_spacing(layout, "lg")
    # Заголовок (прижат к верху)title = QLabel("📝 Редактор шаблонов")apply_heading_style(title, "h1")title.setStyleSheet("color: #6366F1; margin: 0 0 20px 0; padding: 0;")
    ...
    name_layout = QHBoxLayout()name_layout.addWidget(QLabel("Название:"))
    ...
    subject_layout = QHBoxLayout()subject_layout.addWidget(QLabel("Тема письма:"))
    ...
    # Редактор содержимогоcontent_label = QLabel("Содержимое (текст письма):")content_label.setStyleSheet("font-weight: bold; margin-top: 16px;")
...
С уважением,"
    ...
    self._template_highlighter = HtmlHighlighter(
        self.template_content_edit.document()
    )
    ...
    def _create_new_template(self):"""Создание нового шаблона"""self.template_name_edit.setText("Новый шаблон")self.template_subject_edit.setText("Тема письма")self.template_content_edit.setPlainText("Содержимое шаблона...")

    def _delete_template(self):"""Удаление выбранного шаблона"""
    """Выполняет  delete template."""
    current_item = self.templates_list.currentItem()
        if current_item:
        reply = QMessageBox.question(self,"
        ""Подтверждение",""f"Удалить шаблон '{current_item.text()}'?",
    ...
    def _save_template(self):"""Сохранение текущего шаблона"""
    """Выполняет  save template."""
    name = self.template_name_edit.text().strip()
        if not name:QMessageBox.warning(self, "Ошибка", "Введите название шаблона!")
            return
...
QMessageBox.information(self, "Успех", f"Шаблон '{name}' сохранен!")
    ...
    def _preview_template(self):"""Предпросмотр шаблона"""
    """Выполняет  preview template."""
    content = self.template_content_edit.toPlainText()
    subject = self.template_subject_edit.text()

    # Простая подстановка переменных для демонстрацииpreview_content = content.replace("{{name}}", "Иван Петров")preview_content = preview_content.replace("{{email}}", "ivan@example.com")preview_content = preview_content.replace("{{subject}}", subject)preview_content = preview_content.replace("{{date}}", "23 сентября 2025")

    # Создаем окно предпросмотра
    dialog = QDialog(self)dialog.setWindowTitle("Предпросмотр шаблона")
    dialog.setMinimumSize(600, 500)

    layout = QVBoxLayout(dialog)
preview_label = QLabel("Предпросмотр:")preview_label.setStyleSheet("font-weight: bold; margin-bottom: 10px;")
    ...
    def _load_template(self):"""Загрузка выбранного шаблона"""
    """Выполняет  load template."""
    current_item = self.templates_list.currentItem()
        if current_item:
        # Здесь будет логика загрузки из файла/базы
        # Пока просто заполняем примерными данными
        template_name = current_item.text()
        self.template_name_edit.setText(template_name)self.template_subject_edit.setText(f"Тема для {template_name}")

    def _load_templates(self):"""Загрузка списка существующих шаблонов"""
    """Выполняет  load templates."""
    # Здесь будет логика загрузки из файла/базы
    # Пока добавляем примерные шаблоны
    example_templates = ["Приветственное письмо","""Новости компании","""Специальное предложение","""Подтверждение заказа",""]
    ...
    # Заголовокtitle = QLabel("📋 Подробная статистика")apply_heading_style(title, "h1")title.setStyleSheet("color: #6366F1; padding-bottom: 8px; margin-bottom: 16px;")
    title.setAlignment(Qt.AlignCenter)
    layout.addWidget(title)

    # Фильтры периода
    filters_layout = QHBoxLayout()filters_layout.addWidget(QLabel("Период:"))

    self.stats_period_combo = QComboBox()
    self.stats_period_combo.addItems(["Сегодня","За неделю","За месяц","За 3 месяца",
        "За год", "Все время"]
    ...
    self.total_campaigns_card = StatsCard("Всего кампаний","""15",12.5,"
        "accent = QColor("#8B5CF6"),
        parent = None,theme_manager = self.theme_manager,"
        ")
    self.total_emails_card = StatsCard("Отправлено писем","""1,234",8.3,"
        "accent = QColor("#06B6D4"),
        parent = None,theme_manager = self.theme_manager,"
        ")
    self.avg_open_rate_card = StatsCard("Средний Open Rate","""24.7%",2.1,"
        "accent = QColor("#10B981"),
        parent = None,theme_manager = self.theme_manager,"
        ")
    self.avg_click_rate_card = StatsCard("Средний CTR","""3.2%",-0.8,"
        "accent = QColor("#F59E0B"),
        parent = None,
    ...
    # Компактные карточки
        for card in [
        self.total_campaigns_card,
        self.total_emails_card,
        self.avg_open_rate_card,self.avg_click_rate_card,"
        "]:
    ...
    # График отправок по времени (имитация)time_chart_group = QGroupBox("Динамика отправок")
...
📊 Всего за период: 1,032 писем📈 Рост по сравнению с предыдущим периодом: +12.3%"""
    )
    self.time_chart_area.setReadOnly(True)
    time_chart_layout.addWidget(self.time_chart_area)

    # Круговая диаграмма статусовstatus_chart_group = QGroupBox("Статусы доставки")
...
• Блокировка спам-фильтром: 19%• Временная недоступность: 8%"""
    )
    ...
    # Нижняя часть - таблица кампанийcampaigns_table_group = QGroupBox("История кампаний")
    ...
    self.campaigns_table.setHorizontalHeaderLabels(["Название","Дата","Получателей",
        "Отправлено","Успешно", "Ошибок"]
    )

    # Добавляем примерные данные
    sample_campaigns = [("Летняя акция","2025-09-20","500","500","467","33"),
    ...
    common_group_style = ("QGroupBox { border: none; margin-top:0px; padding:0px;} ""QGroupBox::title {left:4px; top:-2px; padding:0 6px; color:#A0AEC0; background: transparent; font-weight:600;}"
    )
    ...
    )  # Получателей по содержимому
    header.setSectionResizeMode(
        3, QHeaderView.ResizeToContents
    )  # Отправлено по содержимому
    header.setSectionResizeMode(
        4, QHeaderView.ResizeToContents
    )  # Успешно по содержимому
    header.setSectionResizeMode(
        5, QHeaderView.ResizeToContents
    )  # Ошибок по содержимому
    # Устанавливаем минимальные размеры
    ...
    self.campaigns_table.setItemDelegateForColumn(
        5, ErrorDotDelegate(self.campaigns_table)
    )
    ...
    self.stats_period_combo.currentTextChanged.connect(
        self._update_statistics_period
    )
    ...
    def _refresh_statistics(self):"""Обновление статистики"""
    """Выполняет  refresh statistics."""
QMessageBox.information(self, "Обновление", "Статистика обновлена!")

    def _update_statistics_period(self, period):"""Обновление статистики при изменении периода"""
    """Выполняет  update statistics period."""
    # Здесь будет логика обновления данных в зависимости от выбранного периода
    pass

    def _export_statistics_csv(self):"""Экспорт статистики в CSV"""
    """Выполняет  export statistics csv."""
    file_path,_ = QFileDialog.getSaveFileName(self,"Сохранить статистику",
        "statistics.csv", "CSV Files (*.csv)"
    )
        if file_path:
        QMessageBox.information(self,"Экспорт", f"Статистика сохранена в {file_path}"
        )

    def _export_statistics_pdf(self):"""Экспорт статистики в PDF"""
    """Выполняет  export statistics pdf."""
    file_path,_ = QFileDialog.getSaveFileName(self,"Сохранить отчет",
        "statistics_report.pdf", "PDF Files (*.pdf)"
    )
        if file_path:QMessageBox.information(self,"Экспорт", f"Отчет сохранен в {file_path}")

    def _create_recipients_page(self):"""Создание страницы управления получателями"""
    """Выполняет  create recipients page."""
    page = QWidget()
    layout = QVBoxLayout(page)apply_margins(layout,"page")apply_spacing(layout, "lg")

    # Заголовокtitle = QLabel("👥 Управление получателями")apply_heading_style(title, "h1")title.setStyleSheet("color: #6366F1; padding-bottom: 8px; margin-bottom: 16px;")
    title.setAlignment(Qt.AlignCenter)
    layout.addWidget(title)
    ...
    # Поиск
    search_layout = QHBoxLayout()search_layout.addWidget(QLabel("Поиск:"))
    self.recipients_search = QLineEdit()self.recipients_search.setPlaceholderText("Введите email или имя...")
    ...
    self.recipients_table.setHorizontalHeaderLabels(["Email","Имя","Группа","Статус", "Дата добавления"]
    )
    ...
    self.recipients_table.setRowCount(len(sample_recipients))
        for row, recipient in enumerate(sample_recipients):
            ...
            # Подкрашиваем статусы
                if col == 3:  # Колонка статусаif value == "Отписался":item.setBackground(QColor("#FEE2E2"))  # Красныйelif value == "Заблокирован":item.setBackground(QColor("#FEF3C7"))  # Желтыйelif value == "Активен":item.setBackground(QColor("#D1FAE5"))  # Зеленый
    ...
    pal = self.recipients_table.palette()pal.setColor(QPalette.Base,
        QColor("#2D3748"))pal.setColor(QPalette.AlternateBase, QColor("#374151"))
    ...
    header.setSectionResizeMode(
        0, QHeaderView.Stretch
    )  # Email - основная растягивающаяся колонка
    header.setSectionResizeMode(
        1, QHeaderView.ResizeToContents
    )  # Имя по содержимому
    header.setSectionResizeMode(
        2, QHeaderView.Stretch
    )  # Группа - дополнительная растягивающаяся
    header.setSectionResizeMode(
        3, QHeaderView.ResizeToContents
    )  # Статус по содержимому
    header.setSectionResizeMode(
        4, QHeaderView.ResizeToContents
    )  # Дата по содержимому
    ...
    self.recipients_table.setColumnWidth(
        2, 200
    )  # Группа - увеличена (будет растягиваться)
    ...
    self.recipients_table.horizontalHeader().setMinimumSectionSize(
        80
    )  # Уменьшен минимум
    self.recipients_table.horizontalHeader().setDefaultSectionSize(
        150
    )  # Размер по умолчанию
    ...
    self.recipients_table.setAlternatingRowColors(
        True
    )  # Чередующиеся цвета строк для лучшей читаемости

        return page

    def _import_recipients(self):"""Импорт получателей из CSV"""
    """Выполняет  import recipients."""
    file_path,_ = QFileDialog.getOpenFileName(self,"Выберите CSV файл","", "CSV Files (*.csv)"
    )
        if file_path:QMessageBox.information(self,"Импорт", f"Импорт из {file_path} завершен!")

    def _export_recipients(self):"""Экспорт получателей в CSV"""
    """Выполняет  export recipients."""
    file_path,_ = QFileDialog.getSaveFileName(self,"Сохранить список получателей",
        "recipients.csv", "CSV Files (*.csv)"
    )
        if file_path:QMessageBox.information(self,"Экспорт", f"Список сохранен в {file_path}")

    def _add_recipient(self):"""Добавление нового получателя"""
    """Выполняет  add recipient."""
    dialog = QDialog(self)dialog.setWindowTitle("Добавить получателя")
    ...
    name_edit = QLineEdit()
    group_combo = QComboBox()group_combo.addItems(["VIP клиенты",
        "Новостная рассылка","Акции", "Партнеры"])
layout.addRow("Email:",email_edit)layout.addRow("Имя:",name_edit)layout.addRow("Группа:", group_combo)
    ...
    layout.addRow(buttons_layout)
        ...
        # Здесь будет логика добавления в базу/файлQMessageBox.information(self, "Успех", "Получатель добавлен!")

    def _delete_recipient(self):"""Удаление выбранного получателя"""
    """Выполняет  delete recipient."""
    current_row = self.recipients_table.currentRow()
        if current_row >= 0:
        email = self.recipients_table.item(current_row, 0).text()
        reply = QMessageBox.question(self,"
        ""Подтверждение",""f"Удалить получателя {email}?",
            QMessageBox.Yes | QMessageBox.No,"
        ")
            if reply == QMessageBox.Yes:
            self.recipients_table.removeRow(current_row)QMessageBox.information(self,
                "Успех", "Получатель удален!")

    def _search_recipients(self, text):"""Поиск получателей"""
    """Выполняет  search recipients."""
    ...
    def _create_advanced_settings_page(self):"""Создание страницы дополнительных настроек"""
    """Выполняет  create advanced settings page."""
    page = QWidget()
    layout = QVBoxLayout(page)apply_margins(layout,"page")apply_spacing(layout, "lg")

    # Заголовокtitle = QLabel("🔧 Дополнительные настройки")apply_heading_style(title, "h1")title.setStyleSheet("color: #6366F1; padding-bottom: 8px; margin-bottom: 16px;")
    title.setAlignment(Qt.AlignCenter)
    layout.addWidget(title)

    # Scroll area для настроек
    scroll = QScrollArea()
    scroll.setWidgetResizable(True)scroll.setStyleSheet("QScrollArea { border: none; }")

    settings_widget = QWidget()
    settings_layout = QVBoxLayout(settings_widget)
    settings_layout.setSpacing(20)

    # Группа: Внешний видappearance_group = QGroupBox("🎨 Внешний вид")
    appearance_layout = QGridLayout(appearance_group)

    # Размер шрифтаappearance_layout.addWidget(QLabel("Размер шрифта:"), 0, 0)
    font_size_spin = QSpinBox()
    font_size_spin.setRange(10, 20)
    font_size_spin.setValue(14)font_size_spin.setSuffix(" px")
    appearance_layout.addWidget(font_size_spin, 0, 1)

    settings_layout.addWidget(appearance_group)

    # Группа: Уведомленияnotifications_group = QGroupBox("🔔 Уведомления")
    notifications_layout = QGridLayout(notifications_group)

    # Системные уведомленияnotifications_layout.addWidget(QLabel("Системные уведомления:"), 0, 0)system_notifications_check = SquareCheckBox("Включить")
    system_notifications_check.setChecked(True)
    notifications_layout.addWidget(system_notifications_check, 0, 1)

    # Email уведомленияnotifications_layout.addWidget(QLabel("Email уведомления:"), 1, 0)email_notifications_check = SquareCheckBox("Включить")
    email_notifications_check.setChecked(False)
    notifications_layout.addWidget(email_notifications_check, 1, 1)

    # Email для уведомленийnotifications_layout.addWidget(QLabel("Email для уведомлений:"), 2, 0)
    notification_email_edit = QLineEdit()notification_email_edit.setPlaceholderText("admin@example.com")
    notifications_layout.addWidget(notification_email_edit, 2, 1)

    settings_layout.addWidget(notifications_group)

    # Группа: Расписаниеschedule_group = QGroupBox("⏰ Автоматические рассылки")
    schedule_layout = QGridLayout(schedule_group)

    # Включить расписаниеschedule_layout.addWidget(QLabel("Автоматические рассылки:"), 0, 0)auto_send_check = SquareCheckBox("Включить")
    schedule_layout.addWidget(auto_send_check, 0, 1)

    # Интервал проверкиschedule_layout.addWidget(QLabel("Интервал проверки:"), 1, 0)
    check_interval_spin = QSpinBox()
    check_interval_spin.setRange(1, 60)
    check_interval_spin.setValue(5)check_interval_spin.setSuffix(" мин")
    schedule_layout.addWidget(check_interval_spin, 1, 1)

    # Максимум рассылок в часschedule_layout.addWidget(QLabel("Максимум рассылок в час:"), 2, 0)
    ...
    # Группа: Безопасностьsecurity_group = QGroupBox("🔒 Безопасность")
    security_layout = QGridLayout(security_group)

    # Логированиеsecurity_layout.addWidget(QLabel("Подробное логирование:"), 0, 0)detailed_logging_check = SquareCheckBox("Включить")
    detailed_logging_check.setChecked(True)
    security_layout.addWidget(detailed_logging_check, 0, 1)

    # Хранить логи (дни)security_layout.addWidget(QLabel("Хранить логи:"), 1, 0)
    log_retention_spin = QSpinBox()
    log_retention_spin.setRange(1, 365)
    log_retention_spin.setValue(30)log_retention_spin.setSuffix(" дней")
    security_layout.addWidget(log_retention_spin, 1, 1)

    # API ключ защитаsecurity_layout.addWidget(QLabel("Проверять API ключи:"), 2, 0)api_check = SquareCheckBox("Включить")
    api_check.setChecked(True)
    security_layout.addWidget(api_check, 2, 1)

    settings_layout.addWidget(security_group)

    # Группа: Производительностьperformance_group = QGroupBox("⚡ Производительность")
    performance_layout = QGridLayout(performance_group)

    # Кэшированиеperformance_layout.addWidget(QLabel("Кэширование шаблонов:"), 0, 0)template_cache_check = SquareCheckBox("Включить")
    template_cache_check.setChecked(True)
    performance_layout.addWidget(template_cache_check, 0, 1)

    # Размер кэшаperformance_layout.addWidget(QLabel("Размер кэша:"), 1, 0)
    cache_size_spin = QSpinBox()
    cache_size_spin.setRange(10, 1000)
    cache_size_spin.setValue(100)cache_size_spin.setSuffix(" МБ")
    performance_layout.addWidget(cache_size_spin, 1, 1)

    # Timeout соединенийperformance_layout.addWidget(QLabel("Timeout соединений:"), 2, 0)
    timeout_spin = QSpinBox()
    timeout_spin.setRange(5, 300)
    timeout_spin.setValue(30)timeout_spin.setSuffix(" сек")
    performance_layout.addWidget(timeout_spin, 2, 1)

    settings_layout.addWidget(performance_group)

    # Применяем стили к группамgroup_style = """
        QGroupBox {
            font-weight: bold;
        ...
        QCheckBox {
            color: white;
        }/* Кастомные чекбоксы рисуются вручную (SquareCheckBox),"
        "поэтому indicator не стилизуем */"""

        for group in [
        appearance_group,
        notifications_group,
        schedule_group,
        security_group,performance_group,"
        "]:
    ...
    # Кнопки управления
    buttons_layout = QHBoxLayout()
    save_settings_btn = ModernButton("💾 Сохранить настройки",""variant="primary",
        theme_manager = self.theme_manager,"
        ")
    ...
    def _save_advanced_settings(self):"""Сохранение дополнительных настроек"""
    """Выполняет  save advanced settings."""
    QMessageBox.information(self,"
        ""Сохранение","""Настройки сохранены!\nНекоторые изменения могут потребовать перезапуска приложения.","")

    def _reset_advanced_settings(self):
    """Сброс настроек к значениям по умолчанию"""
    reply = QMessageBox.question(self,"
        ""Подтверждение","""Сбросить все настройки к значениям по умолчанию?",QMessageBox.Yes | QMessageBox.No,"
        ")
        if reply == QMessageBox.Yes:
        QMessageBox.information(self,"Сброс", "Настройки сброшены к значениям по умолчанию!"
        )

    def _export_advanced_settings(self):"""Экспорт настроек в файл"""
    """Выполняет  export advanced settings."""
    file_path,_ = QFileDialog.getSaveFileName(self,"Сохранить настройки",
        "mailing_settings.json", "JSON Files (*.json)"
    )
        if file_path:
        QMessageBox.information(self,"Экспорт", f"Настройки экспортированы в {file_path}"
        )

    def _create_logs_page(self):"""Создание страницы логов"""
    """Выполняет  create logs page."""
    page = QWidget()
...
controls_layout.addWidget(QLabel("Уровень:"))
    ...
    layout.addWidget(self.logs_view, 1)
        ...
        return page
def _choose_file_ft(self, field: FloatingTextField, filter_: str = "All (*.*)"):"""Выбор файла для FloatingTextField"""path, _ = QFileDialog.getOpenFileName(self, "Выберите файл", "", filter_)
    """Выполняет  choose file ft."""
        if path:
        field.setText(path)

    def _start_campaign(self):"""Запуск рассылки"""
    """Выполняет  start campaign."""
        if self._mailer.is_running():
            return

    file_path = self.recipients_path.text().strip()
        if not file_path:self.progress_label.setText("Не выбран файл получателей")
            return

    recipients = self._load_recipients(file_path)
        if not recipients:self.progress_label.setText("Список получателей пуст")
            return

    template = self.template_path.text().strip()subject = self.subject_edit.text().strip() or "Без темы"
        try:concurrency = int(self.concurrent_input.text().strip() or "10")
        except ValueError:
        concurrency = 10
    ...
    self.cancel_btn.setEnabled(True)self.progress_label.setText("Отправка...")
    self.progress_ring.setVisible(True)
    self.progress_ring.setIndeterminate(True)

    def _cancel_campaign(self):"""Отмена рассылки"""
    """Выполняет  cancel campaign."""
        if self._mailer.is_running():
        self._mailer.cancel()self.progress_label.setText("Отмена...")

    def _load_recipients(self, path: str):"""Загрузка получателей из файла"""loaders = {".csv": CSVLoader(), ".xlsx": ExcelLoader(), ".json": JSONLoader()}
    ...
    loader = loaders.get(ext)
        ...
        data = loader.load(path)
            ...
            if errors:logging.getLogger("mailing.gui").warning(f"Отфильтровано некорректных адресов: {len(errors)}"
            )

            email_map = {r.email: r for r in data}
            return [email_map[email] for email in valid if email in email_map]
        except Exception as e:logging.error(f"Ошибка загрузки получателей: {e}")
            return []

    def _update_dashboard(self):"""Обновление статистики Dashboard"""
    """Выполняет  update dashboard."""
        try:
        # Статистика БД
...
total = stats["total"]success = stats["success"]failed = stats["failed"]
        ...
        self.dashboard_sent_card.setValue(str(total))
            ...
            if total > 0:
            success_rate = (success / total) * 100self.dashboard_success_card.setValue(f"{success_rate:.1f}%")
            self.dashboard_success_card.setProgress(success_rate / 100)
        else:self.dashboard_success_card.setValue("0%")
...
self.dashboard_quota_card.setValue(f"{used}/{limit}")
        ...
        recent_events = event_repo.get_recent_events(3)
            ...
            if recent_events:activity_text = "Последние события:\n\n"
                for event in recent_events:event_type = event.get("event_type",
                "unknown") or "unknown"recipient = event.get("recipient", "unknown") or "unknown"activity_text += f"• {event_type} - {str(recipient)}\n"
        else:activity_text = "Нет последних событий"

        self.activity_list.setPlainText(activity_text)

        except Exception as e:logging.error(f"Ошибка обновления dashboard: {e}")

    def _on_section_changed(self, index: int):"""Смена раздела"""
    """Выполняет  on section changed."""
    self.stack.setCurrentIndex(index)

    def _retranslate(self):"""Обновление переводов"""self.setWindowTitle("Система массовой рассылки - Полное управление")
...
order = ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"]
    ...
    """Выполняет  append log line."""
        ...
        colors = {"DEBUG": "#888888","""INFO": "#ffffff" if self.theme_manager.is_dark() else "#000000","""WARNING": "#e3b341","""ERROR": "#d9544d","""CRITICAL": "#ff0000",""}
    ...
    def _on_mailer_progress(self, event: dict):"""Обработчик прогресса рассылки"""stats = event.get("stats", {})success = stats.get("success", 0)failed = stats.get("failed", 0)total = stats.get("total", 0)
    ...
    def _on_mailer_finished(self, stats: dict):"""Обработчик завершения рассылки"""
    """Выполняет  on mailer finished."""
    self.start_btn.setEnabled(True)
    self.cancel_btn.setEnabled(False)self.progress_label.setText("Завершено")
    self.progress_ring.setVisible(False)
logging.getLogger("mailing.gui").info(f"Рассылка завершена: {stats}")

    def _on_mailer_error(self, msg: str, stats: dict):"""Обработчик ошибки рассылки"""
    """Выполняет  on mailer error."""
    self.start_btn.setEnabled(True)
    self.cancel_btn.setEnabled(False)self.progress_label.setText(f"Ошибка: {msg}")
    self.progress_ring.setVisible(False)
logging.getLogger("mailing.gui").error(f"Ошибка рассылки: {msg}")

    def _on_mailer_cancelled(self, stats: dict):"""Обработчик отмены рассылки"""
    """Выполняет  on mailer cancelled."""
    self.start_btn.setEnabled(True)
    self.cancel_btn.setEnabled(False)self.progress_label.setText("Отменено")
    self.progress_ring.setVisible(False)
logging.getLogger("mailing.gui").info("Рассылка отменена")

    def _on_palette_changed(self, palette):"""Обработчик смены палитры"""
