...
@app.route('/')
def index():
    """Главная страница - минимальная версия"""
    return """
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <form action="/test" method="get">
        <div class="form-group">
            <label>📋 Файл получателей:</label>
            <input type="text" name="file" placeholder="samples/recipients.csv" required>
        </div>
        
        <div class="form-group">
            <label>📄 Шаблон письма:</label>
            <input type="text" name="template" placeholder="template.html" required>
        </div>
        
        <div class="form-group">
            <label>✉️ Тема письма:</label>
            <input type="text" name="subject" placeholder="Тестовая рассылка" required>
        </div>
        
        <div class="form-group">
            <label>🚀 Режим:</label>
            <select name="mode">
                <option value="dry-run">🧪 Тестовый (без отправки)</option>
                <option value="real">🚀 Реальная отправка</option>
            </select>
        </div>
    ...
    <div class="info">
        <h3>📁 Доступные файлы в samples/:</h3>
        <ul>""" + "".join([f"<li>{f.name}</li>" for f in (Path(__file__).parent / "samples").iterdir() if f.is_file()]) + """
        </ul>
    </div>
    ...
    <div class="info">
        ...
        <ol>
            ...
            <li>Нажмите "Показать команду CLI"</li>
...
@app.route('/test')
def test():
    """Показ команды для выполнения"""
    from flask import request
    
    file = request.args.get('file', 'samples/recipients.csv')
    template = request.args.get('template', 'template.html')
    subject = request.args.get('subject', 'Тестовая рассылка')
    mode = request.args.get('mode', 'dry-run')
    
    cmd_parts = [
        "python -m mailing.cli",
        f"--file {file}",
        f"--template {template}",
        f'--subject "{subject}"'
    ]
...
<body>
    ...
    <h3>💻 Выполните эту команду в терминале:</h3>
    <div class="command">
        {command}
    </div>
...
def main():
    print("🌐 Запуск минимального веб-интерфейса...")
    print("📍 Откройте в браузере: http://localhost:5000")
    print("⏹  Для остановки нажмите Ctrl+C")
    
    try:
        app.run(host='127.0.0.1', port=5001, debug=True)  # Используем порт 5001
