...
class HtmlHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, document):
        ...
        tag_format.setForeground(QColor('#60A5FA'))  # голубой
        tag_format.setFontWeight(QFont.Bold)

        attr_format = QTextCharFormat()
        attr_format.setForeground(QColor('#FBBF24'))  # янтарный

        value_format = QTextCharFormat()
        value_format.setForeground(QColor('#34D399'))  # зеленый

        comment_format = QTextCharFormat()
        comment_format.setForeground(QColor('#6B7280'))  # серый
        comment_format.setFontItalic(True)

        string_format = QTextCharFormat()
        string_format.setForeground(QColor('#F472B6'))  # розовый

        jinja_format = QTextCharFormat()
        jinja_format.setForeground(QColor('#A78BFA'))  # сиреневый
        jinja_format.setFontWeight(QFont.Bold)

        # Паттерны
        self.rules.append((QRegularExpression(r"</?\b[\w:-]+"), tag_format))  # начало тега
        self.rules.append((QRegularExpression(r"/>"), tag_format))  # self-close
        self.rules.append((QRegularExpression(r">"), tag_format))  # конец тега
        self.rules.append((QRegularExpression(r"\b[\w:-]+(?==)"), attr_format))  # имя атрибута
        self.rules.append((QRegularExpression(r"=\s*\"[^\"]*\""), value_format))  # значение в "+"
        self.rules.append((QRegularExpression(r"=\s*'[^']*'"), value_format))  # значение в ''
        self.rules.append((QRegularExpression(r"\{{2}[^}]*\}{2}"), jinja_format))  # {{ ... }}
    ...
    def highlightBlock(self, text: str):
        ...
        # Подсветка строк ("...") и ('...') отдельно, чтобы не конфликтовало
        for quote_pat in (r'"([^"\\]|\\.)*"', r"'([^'\\]|\\.)*'"):
